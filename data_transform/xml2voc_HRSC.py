...
def box2trainingtype(input_dir_set,t_type,count):
    ...
    for input_dir in input_dir_set:
        img_dir = os.path.join(input_dir,"AllImages")
        ...
        for file in os.listdir(ann_dir):
            if file.endswith(".xml"):
                ...
                root_i=tree.getroot()
                path_=root_i.find("Img_ID").text
                img_w=root_i.find("Img_SizeWidth").text
                img_h=root_i.find("Img_SizeHeight").text
                ...
                objs = root_i.find("HRSC_Objects")
                for obj in objs.findall("HRSC_Object"):
                    cx=float(obj.find("mbox_cx").text)
                    cy=float(obj.find("mbox_cy").text)
                    w= float(obj.find("mbox_h").text)
                    h= float(obj.find("mbox_w").text)
                    angle= float(obj.find("mbox_ang").text)
                    diff_ =obj.find("difficult").text
                    label=obj.find("Class_ID").text
                    if label not in class_list:
                        class_list.append(label)
                    if cx >= float(obj.find("header_x").text) :
                ...
                dir=os.path.join(target_dir,"Annotations",path_+".xml")
                img =cv2.imread(os.path.join(img_dir,path_+".bmp"))
                cv2.imwrite(os.path.join(target_dir,"JPEGImages",path_+".png"),img)
                tree.write(dir)
    ...
    f=open(os.path.join(target_dir,"voc_class.txt"),"w+")
    f.write("VOC_CLASSES = (\n")
    tree = ET.parse(os.path.join(input_dir_set[0],"sysdata.xml"))
    root_c =tree.getroot()
    clss =root_c.find("HRSC_Classes")
    for cls in clss.findall("HRSC_Class"):
        cls_name = cls.find("Class_ID").text
        f.write("\"{}\",\n".format(cls_name))
    f.write(")")
    f.close()
...
def dataset_aug(xmldir,num):
    ...
    for file in os.listdir(xmldir):
        if file.endswith(".xml"):
            ...
            cv2.imwrite(os.path.join(outdir,str(num)+".jpg"),img)           
            num=num+1
...
# a= float(0)
file_list =["/home/rvl224/文件/HRSC2016/Train","/home/rvl224/文件/HRSC2016/Test"]
# file_list =["/home/rvl224/文件/wilbur_data/source/0521"]
type_ = "theta"
