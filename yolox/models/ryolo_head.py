...
from yolox.utils import bboxes_iou, cxcywh2xyxy, meshgrid, visualize_assign ,cal_iou ,decode_box

from .losses import IOUloss ,PIOUloss
...
class RotateHead(nn.Module):
    ...
    def get_losses(
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                rreg_targets = outputs.new_zeros((0, 6))
        ...
        loss_iou =(
            self.piou_loss(torch.cat((bbox_preds,ang_preds),dim=2).view(-1, 8)[fg_masks], rreg_targets)
        ).sum() /num_fg
        ...
        reg_weight = 5.0
        loss_ang=5.0*loss_direct +2.0*loss_R
        # loss_ang=0.0
    ...
    def get_assignments(
        ...
        rbbox_preds = torch.cat((bboxes_preds_per_image,angl_preds_[:,0:2]),dim=1)
        ...
        gt_rbbox = torch.cat((gt_bboxes_per_image,gt_angles[:,:2]),dim=1)
        pair_wise_ious = bboxes_iou(gt_bboxes_per_image, bboxes_preds_per_image, False)
        # print("pred:")
        # print(rbbox_preds)
        pair_wise_ious= cal_iou(gt_rbbox,rbbox_preds)
        gt_cls_per_image = (
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            direct_loss = F.binary_cross_entropy(
                angl_preds_[:,2:4].sigmoid_().unsqueeze(0).repeat(num_gt, 1, 1),
